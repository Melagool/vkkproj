  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
          {(() => {
              switch (price){
                  case '0': return <Link className="Place__order">Выберите товар</Link>;
                  default: return <Link to={`/basket/${area.id}/${item.id}`} className="Place__order"> Оформить заказ ({price})</Link>;
              }
